...
//获取公司信息，主要是地区信息
const getCompanInfo=async function(companyId)
{
...
const match= async function () {
  //获取用户数据
  let result=await this.getResumeInfo(1);  //获取用户所有简历信息
  this.calculate(result);   //计算用户期望的平均工资


  //计算职位匹配度，主要是工资
    let salary=await positionService.getPositionById(1);
    ...
    //计算公司匹配，主要是地区
    let region=await getCompanInfo(salary.company_id);  //公司所在地
    console.log(region);
